...
api.prototype.validObjectId = function (id) {
	return this.db.ObjectId.isValid(id.toString());
};
...
api.prototype.ent_get = function (id, fn) {
	...
	var self = this;
	if (!self.validObjectId(id)) return fn('Ungültige ID');
...
api.prototype.ent_multitags = function (mode, tag, ids, fn) {
	...
	var _ids = ids.map(function (id) {
		return
		(self.validObjectId(id)) ? self.db.ObjectId(id) : null;
	}).filter(function (id) {
		return id;
	});
	var action = {};
...
api.prototype.rel_check = function (data, fn) {
	...
	data.entities = data.entities.map(function (entity) {
		...
		return self.validObjectId(entity);
	});
	...
	rel.entities = data.entities.map(function (entity) {
		return self.db.ObjectId(entity);
	});
	...
	// check type (no longer used)
	if (!data.hasOwnProperty("type") || typeof data.type !== "string" || data.type === "") rel.type = 'general';
	else rel.type = data.type;
	...
	if (data.hasOwnProperty("data") && (data.data instanceof Array)) {
		data.data.forEach(function (dataset) {
			...
			if (!dataset.hasOwnProperty("key") || (typeof dataset.key !== "string") || dataset.key === "") return;
			if (!dataset.hasOwnProperty("value")) return;
			// description no longer used in relation data if (!dataset.hasOwnProperty("desc") || (typeof dataset.desc !== "string")) return;
			if (!dataset.hasOwnProperty("format") || (typeof dataset.format !== "string") || dataset.format === "") return; // FIXME: proper format check
			if (!dataset.hasOwnProperty("auto") || (typeof dataset.auto !== "boolean")) dataset.auto = false;
			if (!dataset.hasOwnProperty("created")) dataset.created = new Date();
			else dataset.created = vali_date_date(dataset.created).date || new Date();
...
api.prototype.rel_find = function (ents, fn) {
	...
	ents = ents.map(function (id) {
		return self.validObjectId(id) ? self.db.ObjectId(id) : null;
	}).filter(function (id) {
		return id !== null;
	});
	// get entities
...
api.prototype.rel_replace = function (a, b, fn) {
	...
	if ((!self.validObjectId(a)) || (!self.validObjectId(b))) return fn('Ungültige ID');
...
api.prototype.ent_export = function (fn) {
	...
	self.db.collection("relations").find(function (err, rels) {
		if (err) return fn(err);
		if (self.stats) self.stats.relations = rels.length;
		...
		self.db.collection("entities").find(function (err, ents) {
			if (err) return fn(err);
			if (self.stats) self.stats.entities = ents.length;
			ents.forEach(function (ent) {
				if (!(relmap.hasOwnProperty(ent._id.toString()))) return;
				var names = relmap[ent._id.toString()][0];
				var displayname;
				ent.data.forEach(function (d) {
					if ((d.key == 'displayname') && d.value && (typeof d.value == 'string') && (d.value.length > 0)) {
						displayname = d.value;
					}
				});
				if (ent.name !== displayname) names.push(ent.name);
				ent.aliases.forEach(function (alias) {
					if ((names.indexOf(alias) < 0) && (alias !== displayname)) names.push(alias);
				});
	...
	self.db.collection("relations").find({"entities": {"$in": [self.db.ObjectId(id)]}}, function (err, result) {
		...
		result.forEach(function (rel) {
			rel.entities.filter(function (entid) {
				return (entid.toString() !== id.toString()) && self.validObjectId(entid.toString());
	...
	if (query.type == 'relation') {
		self.db.collection("relations").aggregate(
			{$unwind: "$data"},
			{
				$match: {
					"data.key": query.key,
					"data.value": new RegExp(utils.escapeForRegExp(query.q), "i")
				}
			},
			{
				$group: {
...
api.prototype.update_ent_update = function (id, data, fn) {
	...
	self.update_ent_clean(data, function (changed) {
		...
		self.db.collection("update-entities").update({_id: data._id}, data, {upsert: true}, function (err) {
			if (err) return fn(err);
			...
			self.update_entity_info_id(id, function (err, update) {
				...
				self.update_clean_ent_rels(update, function (err) {
					if (err) return fn(err);
					self.update_check_update_needed(update, fn);
				})
			});
		});
	});
...
api.prototype.update_ent_choose = function (id, entid, fn) {
	...
	self.update_ent(id, function (err, entity) {
		...
		self.ent_get(entid, function (err, match) {
			...
			entity.match = match._id.toString();
...
api.prototype.update_ent_create = function (id, fn) {
	...
	self.update_ent(id, function (err, entity) {
		...
		self.update_ent_create_internal(entity, function (err) {
			if (err) return fn(err);
			self.update_entity_info(entity, function (err, update) {
				if (err) return fn(err);
				self.update_check_update_needed(update, fn);
			});
		});
...
api.prototype.update_ent_apply = function (id, data_id, fn) {
	var self = this;
	self.update_entity_info_id(id, function (err, update) {
		if (err) return fn(err);
		if (update.entity.data.length == 0) return fn('Cannot merge, no data');
		...
		if (data_id) {
			...
			update.entity.data = update.entity.data.filter(function (d) {
				return d.id != data_id;
			});
			update.data_matches = update.data_matches.filter(function (d) {
...
api.prototype.update_rel_apply = function (id, data_id, fn) {
	var self = this;
	debug('[update_rel_apply] get update relation : %s - data id:', id, data_id);
	self.update_rel(id, function (err, rel) {
		...
		self.update_rel_info(rel, function (err, update) {
			...
			async.forEachSeries(update.entities, function (entity, next) {
				debug('[update_rel_apply] get update relation entities infos: %s', entity._id);
				self.update_entity_info(entity, function (err, e_update) {
					if (err) return fn(err);
					var update_rel_info = e_update.relations.filter(function (rel_info) {
						return rel_info.rel._id == id;
					})[0];
					if (!update_rel_info) {
						console.log('[update_rel_apply] relation not longer available: %s', id);
						return self.update_check_update_needed(e_update, function (err, e_update) {
							result.push(e_update);
							next();
						});
					}
					var dest_rel = update_rel_info.matches[0];
					if (!dest_rel) return next();

					if (!data_id) {
						dest_rel.data = dest_rel.data.concat(update_rel_info.rel.data);
						debug('[update_rel_apply] updating relation data: %s', dest_rel._id);
						self.rel_update(dest_rel._id, dest_rel, function (err) {
							if (err) return fn(err);
							debug('[update_rel_apply] delete update relation: %s', id);
							self.update_rel_delete_internal(id, function (err) {
								if (err) return fn(err);
								e_update.relations = e_update.relations.filter(function (rel_info) {
									return rel_info.rel._id !== update_rel_info.rel._id;
								});
								self.update_check_update_needed(e_update, function (err, e_update) {
									result.push(e_update);
									next();
								});
							})
						...
						var d = rel.data.filter(function (d) {
							return d.id == data_id;
						})[0];
						if (!d) return next();
						dest_rel.data.push(d);
						debug('[update_rel_apply] adding relation data: %s - %s', dest_rel._id, d.format);
						self.rel_update(dest_rel._id, dest_rel, function (err) {
							if (err) return fn(err);
							update_rel_info.rel.data = update_rel_info.rel.data.filter(function (data) {
								return data.id !== d.id;
							});
							if (update_rel_info.rel.data.length == 0) {
								debug('[update_rel_apply] delete update relation: %s', id);
								self.update_rel_delete_internal(id, function (err) {
									if (err) return fn(err);
									e_update.relations = e_update.relations.filter(function (rel_info) {
								...
								debug('[update_rel_apply] saving update relation: %s', id);
								self.update_rel_store(update_rel_info.rel, function (err) {
									if (err) return fn(err);
									update_rel_info.rel.data_matches = update_rel_info.rel.data_matches.filter(function (data) {
										return data.data.id !== d.id;
									});
									result.push(e_update);
									next();
								});
...
api.prototype.update_rel_store = function (rel, fn) {
	var self = this;
	self.db.collection("update-relations").save(rel, function (err, result) {
		if (err) return fn(err);
		rel._id = result._id;
		debug("update rel stored: %s", result._id);
		fn(null, result._id, rel);
	});
};

api.prototype.update_check_update_needed = function (update, fn) {
	debug('check if update is needed: %s', update.entity._id);
	var self = this;
	...
	if (update_needed()) {
		...
		self.update_ent_delete(update.entity._id, function (err) {
			if (err) return fn(err);
			fn(null, {_id: update.entity._id, deleted: true});
		});
	}
};
...
api.prototype.update_entity_info = function (entity, fn) {
	...
	/*
	 returns update informations
	 {
	 entity: //the given update entity
	 matches: //array of matching db entities
	 data_matches: // array of db relations data
	 [data: ,//the data
	 match, //matching data or null
	 ]
	 relations: // array of relations update informations
	 [{
	 rel: // a update relation
	 entity: // the corresponding update entity
	 entity_matches: //array of matching db entities
	 matches: // array of matching db relations
	 data_matches: // array of db relations data
	 [data: ,//the data
	 match, //matching data or null
	 ]
	...
	var fillRelationDataMatches = function (relinfo) {
		relinfo.data_matches = relinfo.rel.data.map(function (d1) {
			var match = null;
			relinfo.matches.forEach(function (r) {
				if (match) return;
				r.data.forEach(function (d2) {
					if (match) return;
					// only rel1.data needs to be in rel2.data, rel2 may contain more info
					if (utils.field_in_field(d1, d2)) match = d2;
				});
			});
			return {data: d1, match: match};
		});
	};

	var fillEntityDataMatches = function () {
		result.data_matches = result.entity.data.map(function (d1) {
			var match = null;
			result.matches.forEach(function (e) {
				if (match) return;
				e.data.forEach(function (d2) {
					if (match) return;
					// only entity1.data needs to be in entity2.data, entity2 may contain more info
					if (utils.field_in_field(d1, d2)) match = d2;
				});
			});
			return {data: d1, match: match};
	...
	var fillRelations = function (cb) {
		...
		self.update_ent_rels(entity._id, function (err, relations) {
			...
			async.forEachSeries(relations, function (rel, next) {
				var relinfo = {rel: rel, matches: [], entity: null, entity_matches: []};
				result.relations.push(relinfo);
				async.forEachSeries(rel.entities.filter(function (entid) {
					return (entid.toString() !== entity._id.toString());
				}), function (entid, nextID) {
					self.update_ent(entid, function (err, entity) {
						if (err) return fn(err);
						relinfo.entity = entity;
						debug('[update entity info] get matching entities for update relations-entities');
						self.update_match(entity, function (err, matches) {
							if (err) return fn(err);
							relinfo.entity_matches = matches;
							nextID();
						});
					});
				}, function () {
					relinfo.matches = [];
					if ((result.matches.length == 1) && (relinfo.entity_matches.length == 1)) {
						debug('[update entity info] we have a matching entity, so search for matching relations');
						self.rel_find([result.matches[0]._id, relinfo.entity_matches[0]._id], function (err, rel_matches) {
							if (err) return fn(err);
							relinfo.matches = rel_matches;
							fillRelationDataMatches(relinfo);
							next();
						});
					} else {
						fillRelationDataMatches(relinfo);
...
api.prototype.update_clean_ent_rels = function (update, fn) {
	...
	async.forEachSeries(update.relations, function (rel_info, next) {
		if (rel_info.matches.length !== 1) return next();
		debug('clean update relation', rel_info.rel._id);
		self.update_rel_delete_internal(rel_info.rel._id, function (err) {
			if (err) return fn(err);
			update.relations = update.relations.filter(function (r) {
				return rel_info.rel !== r.rel;
			});
			next();
		})
	}, fn);
...
api.prototype.update_intermed = function (intermed, fn) {
	...
	var clean_rels = function (entity, cb) {
		...
		self.update_entity_info_id(entity._id, function (err, update) {
			...
			self.update_clean_ent_rels(update, function (err) {
				if (err) return cb(err);
				self.update_check_update_needed(update, function (err, up) {
					if (err) return cb(err);
					if (up.deleted)
						console.log('update not needed, deleted', entity._id);
					cb();
				});
			})
		});
