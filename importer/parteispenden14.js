...
var debug = require("debug")("importer:parteispenden14");
...
// load data
var data = JSON.parse(fs.readFileSync(path.resolve(__dirname, "data/parteispenden14.json")));
...
var execute = function(finish){
	...
	data.forEach(function(set){
		...
		// create entity
		var ent = {
			importer: "parteispenden14",
			created: (new Date()),
			updated: (new Date()),
			type: ((set.type === "person") ? "person" : "entity"),
			tags: "parteispenden14",
			name: set.name,
			aliases: [set.aliase].filter(function(a){ return (a !== ""); }),
			data: [{
				key: "source",
				value: {
					url: set.source_url,
		...
		if (set.title !== "") set.title.split(/\s+/g).forEach(function(t){
			ent.data.push({
				key: "titles",
				value: t,
				desc: "Titel",
...
if (module.parent === null) {
	...
	api.purge("parteispenden14", function(){
		execute(function(){
